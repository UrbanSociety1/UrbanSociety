...
async function cambiarEstadoPedido(id,estado){try{await Backendless.Data.of(ADMIN_ORDERS_TABLE).save({objectId:id,status:estado});mostrarNotificacionAdmin("Estado actualizado.");await cargarPedidosAdmin();}catch(error){console.error(error);alert("No se pudo actualizar el pedido.");}}
