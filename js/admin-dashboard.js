...
(function () {
  ...
  function localDayBounds(){const n=new Date(),start=new Date(n.getFullYear(),n.getMonth(),n.getDate(),0,0,0,0),end=new Date(n.getFullYear(),n.getMonth(),n.getDate()+1,0,0,0,0);return{start:start.toISOString(),end:end.toISOString()};}
  ...
  function renderActivity(activity){const c=document.getElementById('admin-dashboard-activity');if(!c)return;const rows=Array.isArray(activity)?activity:[];if(!rows.length){c.innerHTML='<div class="admin-dashboard-empty">Todavía no hay actividad registrada.</div>';return;}c.innerHTML=rows.map(i=>`<div class="admin-dashboard-activity-row"><span class="admin-dashboard-activity-icon">${escapeHtml(i.icon||'•')}</span><div><strong>${escapeHtml(i.title||'Actividad')}</strong><small>${escapeHtml(i.subtitle||'')} · ${escapeHtml(formatDate(i.date))}</small></div><strong class="admin-dashboard-activity-amount">${money(i.amount)}</strong></div>`).join('');}

  async function loadDashboard(){if(!window.urbanSupabase)return;const refresh=document.getElementById('admin-dashboard-refresh');if(refresh){refresh.disabled=true;refresh.textContent='Actualizando...';}try{const bounds=localDayBounds();const{data,error}=await window.urbanSupabase.rpc('get_dashboard_summary',{p_day_start:bounds.start,p_day_end:bounds.end});if(error)throw error;const summary=data||{};summary.lowStock=Array.isArray(summary.lowStock)?summary.lowStock:[];renderMetrics(summary);renderLowStock(summary.lowStock);renderActivity(summary.recentActivity);}
