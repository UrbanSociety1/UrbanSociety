const CACHE='urban-society-v3';
const CORE=['./UrbanSociety.html','./account.html','./css/styles.css','./css/polish.css','./images/Logo Urban.png','./manifest.webmanifest'];
self.addEventListener('install',event=>{event.waitUntil(caches.open(CACHE).then(cache=>cache.addAll(CORE.filter(Boolean))).catch(()=>{}));self.skipWaiting();});
